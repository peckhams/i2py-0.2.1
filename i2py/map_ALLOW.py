...
#----------------------------------------------------------------
# (SDP) Need to make sure this happens for use of "bunch", etc.
#----------------------------------------------------------------
_extracode = ['import idl_func']   # List of extra code strings
...
class SubroutineMapping(Mapping):
   ...
   def __init__(self, name, pyname=None, function=False,
      ...
      module code (e.g. import statements, variable and function definitions).
      ...
      self.RETURN_LINE = ''    # (6/29/09) Need this up here, too?
      self.ALLOW_FUNC_OUTVALS = False
      ...
      #--------------------------------------------------------------
      # (SDP) Note that we can't simply add outpars to a tuple of
      # return values because of how functions are typically used.
      # IDL functions with outpars may need to be rewritten.
      #--------------------------------------------------------------
      if not(self.ALLOW_FUNC_OUTVALS):
      ...
      #--------------------------------------------------------------
      # (SDP) Note that we can't simply add outkeys to a tuple of
      # return values because of how functions are typically used.
      # IDL functions with outkeys may need to be rewritten.
      #--------------------------------------------------------------
      if not(self.ALLOW_FUNC_OUTVALS):
   ...
   def pydef(self, pars=(), keys=()):
	 ...
	 body += ' - [%s].count(None)' % ', '.join(in_optional)
      ...
      #--------------------------------------------------
      # (SDP) This is now done below, only when needed.
      #--------------------------------------------------
##      for par in out_only:
##         body += '\n%s = None' % par
	 ...
	 body += ')'
      ...
      if not(self.function) or (self.ALLOW_FUNC_OUTVALS):
	 #
	 ...
	 #
         #-------------------------------------------------------------
	 # (SDP) Modified so that nothing is added in the case where
	 # there is nothing to be returned, which is the case:
	 #   not(out) and not(out_optional).  Also needed to make a
	 # corresponding change in "ir.py" where we have:
	 #     body += '\nreturn _ret()\n'
	 # A parameter called "RETURN_LINE" was added to "self" that
	 # allows the appropriate line to be tacked onto the end of
	 # a translated procedure's body.
	 #-------------------------------------------------------------
##	 print 'pars         =', pars
##	 print 'self.outpars =', self.outpars
##	 print 'self.inpars  =', self.inpars
##	 print 'out          =', out
##	 print 'out_optional =', out_optional

         self.RETURN_LINE = ''    # (default for a procedure)
         
	 if (out or out_optional):
            if (out and not(out_optional)):
	       #-------------------------------------
	       # Output values but no optional ones
	       #------------------------------------------------
	       # (SDP) Experiment to produce cleaner code.
	       # In this case, save the "return line" as a
	       # data member of the SubroutineMapping instance
	       # and then add it to the "body" in the part of
	       # "ir.py" that begins with "if self.PRO"
	       #------------------------------------------------
	       if len(out) == 1:
                  self.RETURN_LINE = 'return %s' % out[0]
	       else:
	          self.RETURN_LINE = 'return (%s)' % ', '.join(out)
            else:
               self.RETURN_LINE = 'return _ret()'
               body += '\n\n'
               body += '#---------------------------------\n'
               body += '# Process optional return values  \n'
               body += '#---------------------------------\n'
               for par in out_only:
                   body += '%s = None\n' % par    
               body += 'def _ret():'
 	       # Output values, some or all of which are optional
	       retbody = '_opt_rv = zip(_opt, [%s])\n' % ', '.join(out_optional)
	       if out:
	          retbody += '_rv = [%s]\n_rv += ' % ', '.join(out)
	       else:
	          retbody += '_rv = '
	       retbody += '[_o[1] for _o in _opt_rv if _o[0] is not None]'
	       #---------------------------------------
	       # (SDP) Don't use tuple unless needed.
	       #---------------------------------------
	       retbody += '\nif (len(_rv) == 1):'
	       retbody += '\n    return _rv[0]'
	       retbody += '\nelse:'
	       retbody += '\n    return tuple(_rv)'    
	       #retbody += '\nreturn tuple(_rv)'   # (before 12/2/08)
...
##      body += '\n'
   ...
   def pycall(self, pars=(), keys=()):
      ...
      #---------------------------------------------
      # (SDP) Can check for keywords as shown here
      #---------------------------------------------
      # if (self.name == 'TOTAL'):
      #    print 'keys =', keys
...
#-----------------------------------------------------------
# (SDP) Allow IDL functions to have outpars and outkeys ??
#-----------------------------------------------------------
...
#----------------------------------------------------------------
#  (SDP) In IDL, boolean keywords have values of 1 or 0, which
#  correspond to True and False as well as set and not set.
#  If IDL code sets a keyword to 0, it is meant to be *unset*.
#  If a boolean keyword is set to 0, the intent is False/unset.
#  When I2PY converts IDL routines to Python, keywords that are
#  set to variable names are initialized to None.  Due to these
#  facts, modified code below to check for both 0 or None.
#  Perhaps we should also include False, since users of the
#  translated code may start setting keywords to False.
#  Note also that:
#       (False is not None) equals True
#       (False != 0) equals False
#       ('str' != 0) equals True
#----------------------------------------------------------------
# (SDP) If the call to KEYWORD_SET is wrapped in parentheses,
#  this can result in two pairs, but is harmless.
#----------------------------------------------------------------
map_func('KEYWORD_SET', inpars=[1], readonly=True,
         callfunc=(lambda i,o: ('(%s not in [0,None])' % i[0])) )
##map_func('KEYWORD_SET', inpars=[1], readonly=True,
##         callfunc=(lambda i,o: ('(%s is not None) and (%s != 0)' \
##                   % (i[0], i[0]))) )
##map_func('KEYWORD_SET', inpars=[1], readonly=True,
