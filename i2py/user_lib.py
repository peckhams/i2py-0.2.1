...
#--------------------------------------------------------------
# Note that I2PY-0.1.0 does not support IDL functions that
# have outpars or outkeys.  The map_func() function does not
# have "outpars" or "outkeys" arguments.  I have experimented
# with adding them and returning the extra output values in a
# tuple as is done when translating IDL procedures.  This new
# behavior (maybe not fully functional) can be toggled with
# this line in "map.py":
#     self.ALLOW_FUNC_OUTVALS = True
# However, this does not seem to be a good solution because
# functions are generally expected to return a single value
# and that is how they are used (e.g. Parent_IDs() ...)

# A better solution seems to be to write new functions that
# return the required values.  For example, the Flow_Codes()
# function returns the D8 flow codes, but the MAP, INCS and
# OPPS outkeys are intended to optionally return additional
# values.  We can simply write additional functions that
# return these additional values.

# Another solution would be to change the original IDL
# functions in these cases to IDL procedures, e.g. the
# Flow_Codes() function could become a procedure called
# Get_Flow_Codes() with extra/optional return values.

# Need to add this explanation to the new README document.
#--------------------------------------------------------------
...
map_pro('Update_Flow_Lengths', inpars=range(1,23), outpars=[1],
        noptional=0, inkeys=['REPORT'])
map_func('Filled_DEM', inpars=range(1,4), noptional=0,
         inkeys=['SILENT'])
...
#--------------------------------------
map_func('Machine_Byte_Order', inpars=[], extracode='import sys')
...
map_pro('Make_RTI_File', inpars=[1,2], noptional=0,
        inkeys=['SILENT'])
map_func('Read_RTI_Value', inpars=[1,2], noptional=0,
        inkeys=['UPCASE'])
...
map_pro('Heap_Insert', inpars=range(1,7), noptional=0)
## map_func('Heap_Get_Min', inpars=[1,2,3], noptional=0,
##          outkeys=['MIN_ID'])
map_func('Heap_Min_ID', inpars=[1])
map_func('Heap_Min', inpars=[1,2,3], noptional=0)
