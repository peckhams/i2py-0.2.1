...
def pyname(name):
   ...
   """
   #-------------------------------------------------------------------
   # In "map.py", map_var function uses VariableMapping class which
   # inherits from Mapping class which has method called "pyname"
   # which calls "util.pyname" in "util.py" (this routine).
   #-------------------------------------------------------------------
   # (SDP) If you like the original mixture of upper and lower
   # case in the IDL variable names, commenting out next line helps.
   # Also, change line in "ir.py" with "config.idlnameconv(self.raw)"
   #-------------------------------------------------------------------
